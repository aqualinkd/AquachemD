...
const net_iface *get_first_valid_interface()
{
  ...
  if (found){
    char *protocol, *port;
    url_ptrs(_acdconfig_.listen_address, &protocol, &port);

    if (port != NULL && protocol != NULL) {
      sprintf(info.url, "%.*s://%s:%s", (int)(protocol - _acdconfig_.listen_address), _acdconfig_.listen_address, info.ip, port);
      sprintf(info.localurl, "%.*s://localhost:%s", (int)(protocol - _acdconfig_.listen_address), _acdconfig_.listen_address, port);
    } else {
      LOG(LOG_ERR,"Could not understand URL '%s' (unable to get protocol & port) \n",_acdconfig_.listen_address);
    }

    if (_acdconfig_.listen_address[(int)(protocol - _acdconfig_.listen_address)-1] == 's' ||
        _acdconfig_.listen_address[(int)(protocol - _acdconfig_.listen_address)-1] == 'S')
      info.isLocalurlTLS = true;
    else
      info.isLocalurlTLS = false;

    LOG(LOG_DEBUG, "Interface %s = %s, %s\n", info.name, info.ip, info.mac);
  } else {
    LOG(LOG_ERR,"Could not find a valid network interface");
  }
